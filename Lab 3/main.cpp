...
const char smallbrickTexFile[] = "images/brick.jpg";
...
//    Concatenate the execution path to the image paths so that fopen() works.
    ...
    std::string smallbrickPath = execPath + "/" + std::string(smallbrickTexFile);
...
// This texture object is for the spots texture map.
 ...
 // This texture object is for the spots texture map.
    ...
    glGenTextures(1, &smallbrickTexObj);
    glBindTexture(GL_TEXTURE_2D, smallbrickTexObj);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);

    if (ReadImageFile(smallbrickPath.data(), &imageData,
        &imageWidth, &imageHeight, &numComponents) == 0) exit(1);
    if (numComponents != 3)
    {
        fprintf(stderr, "Error: Texture image is not in RGB format.\n");
...
{
    glPushMatrix();
    glTranslatef(TABLETOP_X2 / 3, TABLETOP_Y1 * 2 / 3, 0);
    // Top
    ...
    glNormal3f(0.0, 0.0, 1.0); // Normal vector.
    SubdivideAndDrawQuad(8, 8, 0.0, 1.0, TABLETOP_X2 / 8, TABLETOP_Y1 / 8, TABLETOP_Z + 0.3,
        1.0, 1.0, TABLETOP_X2 / 8, TABLETOP_Y2 / 8, TABLETOP_Z + 0.3,
        1.0, 0.0, TABLETOP_X1 / 8, TABLETOP_Y2 / 8, TABLETOP_Z + 0.3,
        0.0, 0.0, TABLETOP_X1 / 8, TABLETOP_Y1 / 8, TABLETOP_Z + 0.3);
    ...
    glNormal3f(0.0, 0.0, -1.0); // Normal vector.
    SubdivideAndDrawQuad(8, 8, 0.0, 0.0, TABLETOP_X1 / 8, TABLETOP_Y1 / 8, TABLETOP_Z + 0.1,
        1.0, 0.0, TABLETOP_X1 / 8, TABLETOP_Y2 / 8, TABLETOP_Z + 0.1,
        1.0, 1.0, TABLETOP_X2 / 8, TABLETOP_Y2 / 8, TABLETOP_Z + 0.1,
        0.0, 1.0, TABLETOP_X2 / 8, TABLETOP_Y1 / 8, TABLETOP_Z + 0.1);
    ...
    GLfloat matSpecular2[] = { 0.2, 0.2, 0.2, 1.0 };
    GLfloat matShininess2[] = { 8.0 };
    glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, matAmbient2);
    glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, matDiffuse2);
    glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, matSpecular2);
    glMaterialfv(GL_FRONT_AND_BACK, GL_SHININESS, matShininess2);

    glBindTexture(GL_TEXTURE_2D, smallbrickTexObj); // Texture object ID == 0 means no texture mapping.

    // In +y direction.
    glNormal3f(0.0, 1.0, 0.0); // Normal vector.
    SubdivideAndDrawQuad(8, 4, 0.0, 0.0, TABLETOP_X2 / 8, TABLETOP_Y2 / 8, TABLETOP_Z + 0.1,
        1.0, 0.0, TABLETOP_X1 / 8, TABLETOP_Y2 / 8, TABLETOP_Z + 0.1,
        1.0, 1.0, TABLETOP_X1 / 8, TABLETOP_Y2 / 8, TABLETOP_Z + 0.3,
        0.0, 1.0, TABLETOP_X2 / 8, TABLETOP_Y2 / 8, TABLETOP_Z + 0.3);
    // In -y direction.
    glNormal3f(0.0, -1.0, 0.0); // Normal vector.
    SubdivideAndDrawQuad(8, 4, 0.0, 0.0, TABLETOP_X1 / 8, TABLETOP_Y1 / 8, TABLETOP_Z + 0.1,
        1.0, 0.0, TABLETOP_X2 / 8, TABLETOP_Y1 / 8, TABLETOP_Z + 0.1,
        1.0, 1.0, TABLETOP_X2 / 8, TABLETOP_Y1 / 8, TABLETOP_Z + 0.3,
        0.0, 1.0, TABLETOP_X1 / 8, TABLETOP_Y1 / 8, TABLETOP_Z + 0.3);
    // In +x direction.
    glNormal3f(1.0, 0.0, 0.0); // Normal vector.
    SubdivideAndDrawQuad(8, 4, 0.0, 0.0, TABLETOP_X2 / 8, TABLETOP_Y1 / 8, TABLETOP_Z + 0.1,
        1.0, 0.0, TABLETOP_X2 / 8, TABLETOP_Y2 / 8, TABLETOP_Z + 0.1,
        1.0, 1.0, TABLETOP_X2 / 8, TABLETOP_Y2 / 8, TABLETOP_Z + 0.3,
        0.0, 1.0, TABLETOP_X2 / 8, TABLETOP_Y1 / 8, TABLETOP_Z + 0.3);
    // In -x direction.
    glNormal3f(-1.0, 0.0, 0.0); // Normal vector.
    SubdivideAndDrawQuad(8, 4, 0.0, 0.0, TABLETOP_X1 / 8, TABLETOP_Y2 / 8, TABLETOP_Z + 0.1,
        1.0, 0.0, TABLETOP_X1 / 8, TABLETOP_Y1 / 8, TABLETOP_Z + 0.1,
        1.0, 1.0, TABLETOP_X1 / 8, TABLETOP_Y1 / 8, TABLETOP_Z + 0.3,
        0.0, 1.0, TABLETOP_X1 / 8, TABLETOP_Y2 / 8, TABLETOP_Z + 0.3);
