...
void DefineScene2( Scene &scene, int imageWidth, int imageHeight )
{
	...
	scene.numSurfaces = 23;
	scene.surfacep = new SurfacePtr[scene.numSurfaces];

	int counter = 0;

	scene.surfacep[counter++] = new Plane(0.0, 1.0, 0.0, 0.0, &(scene.material[0])); // Horizontal plane.
	scene.surfacep[counter++] = new Plane(1.0, 0.0, 0.0, 0.0, &(scene.material[0])); // Left vertical plane.
	scene.surfacep[counter++] = new Plane(0.0, 0.0, 1.0, 0.0, &(scene.material[0])); // Right vertical plane.

	//sphere
	double radius = 12.0;
	double spheretrans_X = sqrt(pow(2 * radius, 2) - pow(radius, 2));

	// 10 surfaces
	// first layer
	for (int i = 0; i < 4; i++) {
		double spheretrans_Z = radius * i * 2;
		scene.surfacep[counter++] = new Sphere(Vector3d(75.0, 10.0, 40.0 + spheretrans_Z), radius, &(scene.material[1]));
	}
	for (int i = 0; i < 3; i++) {
		double spheretrans_Z = radius * i * 2;
		scene.surfacep[counter++] = new Sphere(Vector3d(75.0 + spheretrans_X, 10.0, 52.0 + spheretrans_Z), radius, &(scene.material[2]));
	}
	for (int i = 0; i < 2; i++) {
		double spheretrans_Z = radius * i * 2;
		scene.surfacep[counter++] = new Sphere(Vector3d(75.0 + 2 * spheretrans_X, 10.0, 64.0 + spheretrans_Z), radius, &(scene.material[3]));
	}
	scene.surfacep[counter++] = new Sphere(Vector3d(75.0 + 3 * spheretrans_X, 10.0, 76.0), radius, &(scene.material[4]));

    // Cube +y face.
    scene.surfacep[counter++] = new Triangle(Vector3d(50.0, 20.0, 90.0), Vector3d(50.0, 20.0, 70.0),
        Vector3d(30.0, 20.0, 70.0), &(scene.material[5]));
    scene.surfacep[counter++] = new Triangle(Vector3d(50.0, 20.0, 90.0), Vector3d(30.0, 20.0, 70.0),
        Vector3d(30.0, 20.0, 90.0), &(scene.material[5]));

    // Cube +x face.
    scene.surfacep[counter++] = new Triangle(Vector3d(50.0, 0.0, 70.0), Vector3d(50.0, 20.0, 70.0),
        Vector3d(50.0, 20.0, 90.0), &(scene.material[5]));
    scene.surfacep[counter++] = new Triangle(Vector3d(50.0, 0.0, 70.0), Vector3d(50.0, 20.0, 90.0),
        Vector3d(50.0, 0.0, 90.0), &(scene.material[5]));

    // Cube -x face.
    scene.surfacep[counter++] = new Triangle(Vector3d(30.0, 0.0, 90.0), Vector3d(30.0, 20.0, 90.0),
        Vector3d(30.0, 20.0, 70.0), &(scene.material[5]));
    scene.surfacep[counter++] = new Triangle(Vector3d(30.0, 0.0, 90.0), Vector3d(30.0, 20.0, 70.0),
        Vector3d(30.0, 0.0, 70.0), &(scene.material[5]));

    // Cube +z face.
    scene.surfacep[counter++] = new Triangle(Vector3d(50.0, 0.0, 90.0), Vector3d(50.0, 20.0, 90.0),
        Vector3d(30.0, 20.0, 90.0), &(scene.material[5]));
    scene.surfacep[counter++] = new Triangle(Vector3d(50.0, 0.0, 90.0), Vector3d(30.0, 20.0, 90.0),
        Vector3d(30.0, 0.0, 90.0), &(scene.material[5]));

    // Cube -z face.
    scene.surfacep[counter++] = new Triangle(Vector3d(30.0, 0.0, 70.0), Vector3d(30.0, 20.0, 70.0),
        Vector3d(50.0, 20.0, 70.0), &(scene.material[5]));
    scene.surfacep[counter++] = new Triangle(Vector3d(30.0, 0.0, 70.0), Vector3d(50.0, 20.0, 70.0),
        Vector3d(50.0, 0.0, 70.0), &(scene.material[5]));
